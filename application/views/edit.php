<?php $this->load->view('header'); ?>
		...
		<div class="new_invoice_box">
			...
			<!-- New form -->			
			<?php echo form_open(base_url('updateInvoice' ),array( 'id' => '', 'class' => '' ));?>
					<input type="hidden" name="invoice_items" id="invoice_items" value="<?php echo count($this->session->flashdata('data')['invoice_items']); ?>" />
					<input type="hidden" name="invoice_id" value="<?php if(!empty($this->session->flashdata('data'))){ echo $this->session->flashdata('data')['invoice_id']; } ?>" />
					...
					<?php for($i=0;$i<count($this->session->flashdata('data')['invoice_items']);$i++){ $j = $i + 1;?>					
					...
					<div class="row">						
						<div class="col">														
							<button type="submit" class="btn btn-primary btn-md" id="update_btn">Update</button>
						</div>																
					</div>
